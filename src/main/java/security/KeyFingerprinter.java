// Original
// https://github.com/joyent/java-http-signature/blob/master/common/src/main/java/com/joyent/http/signature/KeyFingerprinter.java
...
/**
 * Utility class for calculating and verifying SSH key fingerprints as defined by OpenSSH.
 */
public final class KeyFingerprinter {
     ...
     *
     * @param publicKey The KeyPair to calculate the fingerprint of
     * @return The MD5 fingerprint (ex: {@code 9f:0b:50:ae:e3:da:f6:eb:b5:71:9a:69:ee:79:9e:c2})
     */
    public static String md5Fingerprint ( final PublicKey publicKey ) {
        ...
        byte[] encoded = SshEncoder.encode( publicKey );
        md.update( encoded );
    ...
    private static String colonify ( final byte[] bytes ) {
    ...
    private static class SshEncoder {
        ...
        public static byte[] encode ( final PublicKey key ) {
            if ( key instanceof RSAPublicKey ) {
                ...
                throw new RuntimeException( "unknown public key type: " + key.getClass().getName() );
            }
        }
